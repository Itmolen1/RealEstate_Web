...
class User_model extends CI_Model
{
    ...
    function get_dashboard_data()
    {
        ...
        $query=$this->db->get_where('tbl_building');
        $res=$query->result_array();
        $total_properties=$query->num_rows();
        $data['total_properties']=$total_properties;

        $this->db->where('isDeleted',0);
        $query=$this->db->get_where('tbl_property');
        $res=$query->result_array();
        $total_property_units=$query->num_rows();
        $data['total_property_units']=$total_property_units;

        $this->db->where('isDeleted',0);
        $query=$this->db->get_where('tbl_customer');
        $res=$query->result_array();
        $total_customers=$query->num_rows();
        $data['total_customers']=$total_customers;

        $this->db->where('isDeleted',0);
        $query=$this->db->get_where('tbl_vendor');
        $res=$query->result_array();
        $total_vendors=$query->num_rows();
        $data['total_vendors']=$total_vendors;

        $this->db->select_sum('vat_amt');
        $this->db->where('isDeleted',0);
        $query = $this->db->get('tbl_expence');
        $res=$query->row_array();
        $data['sum_of_vat']=$res['vat_amt'];

        $this->db->select_sum('total_amt');
        $this->db->where('isDeleted',0);
        $query = $this->db->get('tbl_expence');
        $res=$query->row_array();
        $data['sum_of_expence_inc_vat']=$res['total_amt'];

        $this->db->select_sum('expence_amount');
        $this->db->where('isDeleted',0);
        $query = $this->db->get('tbl_expence');
        $res=$query->row_array();
        $data['sum_of_expence_amount_exc_vat']=$res['expence_amount'];

        $this->db->select_sum('vat_amt');
        $this->db->where('isDeleted',0);
        $query = $this->db->get('tbl_po_boi');
        $res=$query->row_array();
        $data['sum_of_input_vat']=$res['vat_amt'];

        $this->db->select_sum('total_amt');
        $this->db->where('isDeleted',0);
        $query = $this->db->get('tbl_po_boi');
        $res=$query->row_array();
        $data['sum_of_income_inc_vat']=$res['total_amt'];

        $this->db->select_sum('rent');
        $this->db->where('isDeleted',0);
        $query = $this->db->get('tbl_po_boi');
        $res=$query->row_array();
        $data['sum_of_income_amount_exc_vat']=$res['rent'];
